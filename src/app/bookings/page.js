...
function BookingsContent() {
  ...
  const confirmCashBooking = async (bookingIds) => {
    ...
    const transactionId = `CASH-${bookingIds.join("-")}`;
    const paymentResponse = await fetch(apiUrl("/payments/save-transaction"), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!paymentResponse.ok) {
      ...
      throw new Error(data.error || "Cash payment record could not be saved");
    }
