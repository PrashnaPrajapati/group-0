...
export default function NotificationSystem({ userId, userRole, onChatCountChange }) {
  ...
  useEffect(() => {
    ...
    newSocket.on("notification_count_updated", () => {
      fetchNotifications();
    });
  ...
  }, [userId, userRole, normalizeNotification, fetchNotifications]); 
  useEffect(() => {
    if (!userId) return;
    fetchNotifications(); 
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.type === "chat_message") {
      ...
      ["reminder", "booking_confirmed", "booking_completed", "booking_missed"].includes(notification.type)
      || notification.type === "booking_received"
    ) {
      setShowDropdown(false);
      router.push("/dashboard");
    } else if (["admin_alert", "cancellation"].includes(notification.type)) {
      setShowDropdown(false);
  ...
  return (
    <div className="relative"> 
      ...
      {showDropdown && (
        <div id={dropdownId} className="absolute right-0 mt-3 w-80 bg-white border rounded-xl shadow-lg z-50 max-h-96 overflow-y-auto"> 
          <div className="flex justify-between items-center p-4 border-b">
            <h3 id="notifications-heading" className="font-semibold text-gray-800">Notifications</h3>
            {unreadCount > 0 && (
              <button
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((notification) => (
                <button
                  key={notification.id}
                  type="button"
                  className={`block w-full p-4 border-b text-left hover:bg-gray-50 ${
